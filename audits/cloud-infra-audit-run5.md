...
| 10 | **CLOUD-023** — Auth secret weakness not caught at build time | Add Railway health check that triggers auth module init; add CI env var validation step | 2 hours | **Scheduled** |
| 11 | **CLOUD-018** — Email verification disabled | Set `requireEmailVerification: !!process.env.RESEND_API_KEY` | 30 minutes | **Immediate** |
| 12 | **CLOUD-019** — No account lockout on login | Implement Redis-backed per-email rate limiter on sign-in endpoint | 3 hours | **Scheduled** |
| 13 | **CLOUD-020** — No staging environment | Create separate Supabase project and Railway environment for staging | 4 hours | **Scheduled** |