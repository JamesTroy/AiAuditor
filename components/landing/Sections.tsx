...
// ProofStrip, HowItWorks, Features, Testimonials — all client components
// after the motion retrofit. They rely on whileInView for scroll-triggered
// reveals as the user scrolls down the landing page.

import { motion } from 'motion/react';
import { fadeUp, staggerContainer, transitions } from '@/lib/motion/variants';

// Shared whileInView config — `once: true` means each section only animates
// the first time it enters the viewport (no re-animation when scrolling
// back up); `margin: '-80px'` waits until the section is meaningfully
// visible before triggering, not the moment its top edge clips the
// viewport.
...
export function ProofStrip() {
  // Note: "94%" stat below is unsourced — needs a real number or removal.
  // Flagged separately from this animation retrofit; leaving content as-is
  // so a copy decision can be made in its own commit.
  const stats = [
    { num: '193', label: 'Specialized auditors' },
  ...
  return (
    <div className="border-y border-zinc-800 bg-zinc-900/50">
      <motion.div
        ...
        whileInView="visible"
        viewport={inView}
        className="max-w-6xl mx-auto px-6 py-7 flex items-center justify-between flex-wrap gap-6"
      >
        {stats.map((s) => (
...
const STEPS = [
  ...
  { n: '02', title: 'Pick your auditors', body: 'Choose from 193 specialized auditors or run all of them. Security, performance, type safety, auth patterns, SQL hygiene — each one is an expert in exactly one thing.' },
    ...
    <section id="how-it-works" className="py-28 border-b border-zinc-800/60">
      <div className="max-w-6xl mx-auto px-6">
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0, transition: transitions.soft }}
          viewport={inView}
        >
...
const FEATURES = [
  ...
  },
  {
    ...
    title: '193 specialized auditors',
    body: 'Each auditor is an expert in one thing. Run all 193 or cherry-pick the categories that matter for your file.',
  },
  {
        ...
        >
          {FEATURES.map((f) => (
            <motion.div
              key={f.title}
              variants={fadeUp}
              whileHover={{ y: -4, transition: transitions.springGentle }}
...
// ── Testimonials ────────────────────────────────────────────────────────────
//
// Note: these testimonials have very specific falsifiable claims (e.g. "score
// went from 71 to 89 in 6 weeks"). They read as fabricated and need to be
// either (a) sourced from real users with consent or (b) removed — FTC
// endorsement rules don't allow fake testimonials. Flagged separately from
// this animation retrofit so a content decision can be made in its own
// commit.
